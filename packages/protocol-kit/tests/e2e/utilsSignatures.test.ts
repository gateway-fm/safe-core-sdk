...
describe('Signature utils', () => {
  describe('isTxHashSignedWithPrefix', () => {
    it('returns false if message was signed without a prefix', async () => {
      ...
      chai.expect(await isTxHashSignedWithPrefix(safeTxHash, signature, ownerAddress)).to.be.false
    })

    it('returns true if message was signed with a prefix', async () => {
      ...
      chai.expect(await isTxHashSignedWithPrefix(safeTxHash, signature, ownerAddress)).to.be.true
    })
  })

  describe('adjustVInSignature', () => {
    it('eth_sign: adjusts V to V > 30 when message is signed with a prefix', async () => {
      const hex27 = '1b'
      const hex31 = '1f'
      ...
      chai
        .expect(
          await adjustVInSignature(SigningMethod.ETH_SIGN, signature, safeTxHash, signerAddress)
        )
        .to.be.eq(adjustedSignature)
    })

    it('eth_sign: adjusts V to V > 30 when message is signed with a prefix and V < 27', async () => {
      const hex01 = '01'
      const hex32 = '20'
    ...
    it("eth_sign: doesn't touch V when message is signed without a prefix and V is one of {27, 28}", async () => {
      const hex27 = '1b'
